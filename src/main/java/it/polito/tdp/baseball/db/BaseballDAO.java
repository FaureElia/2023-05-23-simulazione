...
public class BaseballDAO {
	...
	public List<People> getPeople(int anno, int salario) {
		String sql = "SELECT p.* , MAX(s.salary) salario "
	...
	public List<Coppia> getCoppie(Map<String, People> idMap, int anno, int salario) {
		String sql = "SELECT tmp1.playerID AS p1, tmp2.playerID AS p2 "
				+ "FROM "
				+ "(SELECT a.playerID, a.teamID "
				+ "FROM appearances a, salaries s "
				+ "WHERE a.playerID=s.playerID AND a.`year`=s.`year` AND a.`year`=? AND s.salary>? "
				+ "GROUP BY a.playerID, a.teamCode) tmp1, "
				+ "(SELECT a.playerID, a.teamID "
				+ "FROM appearances a, salaries s "
				+ "WHERE a.playerID=s.playerID AND a.`year`=s.`year` AND a.`year`=? AND s.salary> ? "
				+ "GROUP BY a.playerID, a.teamCode) tmp2 "
				+ "WHERE tmp1.playerID>tmp2.playerID AND tmp1.teamID=tmp2.teamID "
				+ "GROUP BY tmp1.playerID, tmp2.playerID ";
		...
		try {
			...
			st.setInt(2, salario*1000000);
			st.setInt(3, anno);
			st.setInt(4, salario*1000000);
			
			
			ResultSet rs = st.executeQuery();

			while (rs.next()) {
				result.add(new Coppia(idMap.get(rs.getString("p1")),idMap.get(rs.getString("p2"))));
			}
